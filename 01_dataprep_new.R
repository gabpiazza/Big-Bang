...
potential_suppliers_tech <- potential_suppliers %>%
  rename(x3_digits = activitycode) %>% 
  left_join(tech_level)

# The next step is to replace the tech intensity NA and ? with 0 - not making any assumptions on what 0 means
potential_suppliers_tech$tech_intensity[potential_suppliers_tech$tech_intensity=="?"]<-0 
potential_suppliers_tech$tech_intensity[is.na(potential_suppliers_tech$tech_intensity)]<- 0

# I want to assign the high-tech variable to the order 
high_tech_order <- c(3,4,5,7)# I use the information provided by CSIL
low_tech_order<- c(1,2,6,0,9,8, NA) #

potential_suppliers_tech<- potential_suppliers_tech %>% 
  mutate(tech_level= case_when(tech_intensity %in% high_tech_order~1,
                               TRUE ~0))

balance_MS<- balance_MS %>% 
  rename(year_supplier_registration = order_date)
potential_suppliers_tech_balance<- potential_suppliers_tech %>% 
  left_join(balance_MS, by = c("year_supplier_registration", "country"))

saveRDS(potential_suppliers_tech_balance, paste0(data_proc_dir, "potential_suppliers_tech_balance"))
